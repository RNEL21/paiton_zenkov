...
def format_price(price, currency="руб", discount=0):
    ...
    return f"{final:.2f} {currency}".strip()
...
def is_valid(product):
    return product["price"] > 0 and product["rating"] >= 4.0 and product["in_stock"]

valid_products = list(filter(is_valid, products))

with_final_price = list(map(
    lambda p: {**p, "final_price": p["price"] * 0.85},
    valid_products
))

sorted_products = sorted(with_final_price, key=lambda p: p["final_price"])
